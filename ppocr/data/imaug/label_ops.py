...
class PRENLabelEncode(BaseRecLabelEncode):
    ...
    def add_special_char(self, dict_character):
        self.pad_str = "<pad>"
        self.end_str = "<eos>"
        dict_character = [self.pad_str] + [self.end_str] + dict_character
        return dict_character
    ...
    def __call__(self, data):
        ...
        if len(text) > self.max_text_len - 1:
            return None
        data['length'] = np.array(len(text))
        text = text + [1] + [0] * (self.max_text_len - len(text) - 1)
        data['label'] = np.array(text)
        return data

    def get_ignored_tokens(self):
        pad_idx = self.get_beg_end_flag_idx("pad")
        end_idx = self.get_beg_end_flag_idx("end")
        return [pad_idx, end_idx]

    def get_beg_end_flag_idx(self, beg_or_end):
        if beg_or_end == "pad":
            idx = np.array(self.dict[self.pad_str])
        elif beg_or_end == "end":
            idx = np.array(self.dict[self.end_str])
        else:
            assert False, "Unsupport type %s in get_beg_end_flag_idx" \
